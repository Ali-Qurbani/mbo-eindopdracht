...
foreach ($coins as $coin) {
    $statement2 = $db->prepare("UPDATE `coins` SET `visibility` = '1' WHERE `coins`.`id` = :coin_id;", array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
    $statement2->execute(array('coin_id' => $coin->coin_id));
}

$_SESSION['dashboard-alert-type'] = 'success';
$_SESSION['dashboard-message'] = 'Coin category visibility successfully set to visible.';
header("Location: /admin_coins.php");